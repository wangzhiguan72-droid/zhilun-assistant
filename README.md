...
**付费熔断**：`LLM_PAID_MAX_CALLS=200` 给「付费模型 × 项目方 Key」调用设进程内硬上限，
超过后付费候选一律跳过（免费模型与用户自带 Key 不受影响）。多人部署且填了付费
Key 时建议设一个整数，防止账单失控；默认 0 = 不启用。观测：`/api/llm_stats` 的
`paid_brake` 字段。

**完全离线**：Markdown 渲染器（markdown-it）已随包分发在 `static/vendor/`，页面
不依赖任何 CDN——断网时统计结果照常渲染（AI 功能本身需要联网，那是它调模型）。

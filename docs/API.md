...
- **个人账号（v2.27，可选）**：部署者设 `ACCOUNTS_ENABLED=1` 后，所有页面与
  API 还要求**个人账号登录**（未登录：API 回 `401 {"need_login": true}`，页面跳 `/login`）；
  限流改按用户名计数；`file_id` 归属上传者，他人访问 403。未开启时本节全部不存在。
- **数据不落盘**：原始数据全程内存处理。
  `POST /api/audit_image` 上传的**图片同样不落盘**（仅请求内以 base64 送模型）。

---

## 个人账号（v2.27，`ACCOUNTS_ENABLED=1` 时才有）

浏览器流程：口令门禁（`/unlock`）→ 注册/登录（本节）→ 携带两个 cookie 使用。
三个页面路径对**两道门都豁免**（否则死锁）；注册表单自带口令字段，门槛没有降低。

### `POST /register`

- **表单**：`code`（访问口令，邀请制——与门禁同一把）、`username`（2–32 位，
  中文/字母/数字/下划线/连字符/点）、`password`（≥8 位）
- 成功 `302` → `/`，种 `zhilun_user` 签名 cookie；口令错 `401`；规则不符/重名 `400`
- 失败计入按 IP 防爆破锁定（与 `/unlock`、`/login` 共享：5 次失败锁 15 分钟）

### `POST /login`

- **表单**：`username` + `password`。成功 `302` → `/`；失败 `401`；锁定 `429`
- cookie 为「用户名｜过期｜HMAC 签名」，httponly；**签名密钥由口令派生，
  改 `ACCESS_CODE` 全员账号会话立即失效**；有效期 7 天

### `GET /logout-user`

- 清除账号 cookie → `/login`（口令门禁不受影响）

### 对 API 调用方的影响

- 未登录调用任何业务 API：`401 {"ok": false, "error": "请先登录个人账号。", "need_login": true}`
  （与门禁的 `need_unlock: true` 是两层，注意区分）
- `file_id` **归属上传者**：其他账号带同一 `file_id` 调用 → `403`；
  会话有界（4 小时空闲/64 份上限），过期后需重新上传
- `GET /api/limits` 统计里限流按「用户名」分桶（未登录回退按 IP）
